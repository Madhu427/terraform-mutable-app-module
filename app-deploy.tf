resource "null_resource" "app_deploy" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "ansible-pull -U https://github.com/Madhu427/ansible.git roboshop-pull.yml -e COMPONENT=${var.COMPONENT} -e ENV=${var.ENV} -e NEXUS_USERNAME=${local.NEXUS_USERNAME} -e NEXUS_PASSOWRD=${local.NEXUS_PASSWORD}"
    ]
  }
}
